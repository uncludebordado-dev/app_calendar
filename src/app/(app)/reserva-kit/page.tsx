...
export default async function ReservaKitPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold">{t("Reservá tu kit")}</h1>
        <p className="mt-1 text-sm text-piedra">
          {t("Elegí el que va con vos. Al reservar, la profe recibe el pedido y te escribe para coordinar la entrega. El pago se hace en persona.")}
        </p>
      </div>

      {kits.length === 0 ? (
        <p className="rounded-xl border border-dashed border-lino px-4 py-8 text-center text-sm text-piedra">
          {t("Todavía no hay kits cargados.")}
        </p>
      ) : (
      ...
      <p className="pt-2 text-center text-xs text-piedra">
        <Link href="/calendario" className="underline">{t("← Volver al calendario")}</Link>
      </p>
    </div>
  );
}
